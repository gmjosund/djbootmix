...
export const buyOrSellPair = ({commit}, data) => {
  return axios.post(THE_BASE_URL + '/action/' + data.buyOrSell + '?currencyPair=' + data.pair + '&executionType=' + data.executionType).then(response => { 
...
export const enterPending = ({commit}, data) => {
  return axios.post(THE_BASE_URL + '/action/pending?currencyPair=' + data.pairName + '&targetPrice=' + data.targetPrice).then(response => { 
    data.vm.$swal.fire('Submitted order');
  })
  .catch(error => {
...
export const reserveAmount = ({commit}, data) => {
  return axios.post(THE_BASE_URL + '/api/v2/balance/reserve?amount='+ data.amount +'&currencyPair=' + data.pair).then(response => { 
...
export const cancelPending = ({commit}, data) => {
  return axios.post(THE_BASE_URL + '/action/cancel?currencyPair='+data.pairName+'&orderNumber='+data.orderNumber).then(response => { 
    data.vm.$swal.fire('Record is Added Sucessfully');
  })
  .catch(error => {
    if(error.response.status === 304){
